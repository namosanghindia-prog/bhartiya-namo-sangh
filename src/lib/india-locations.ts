/**
 * States, union territories and their districts, for the address pickers on
 * the member forms. Plain module: used by client forms and safe on the server.
 *
 * `name` is what gets stored (members.state, branches.state, …) and must stay
 * in step with STATE_CODES in membership.ts, which keys on it. New districts
 * are carved out every year; the picker's "Other" option covers the gap until
 * a list here is updated.
 */
...
export const INDIA_STATES: readonly IndianState[] = [
  ...
  },
  {
    ...
    kind: "state",
    districts: [
      "Bagalkote", "Ballari", "Belagavi", "Bengaluru Rural", "Bengaluru South (Ramanagara)",
      "Bengaluru Urban", "Bidar", "Chamarajanagar", "Chikkaballapura", "Chikkamagaluru",
...
/** Case- and whitespace-insensitive lookup by English name. */
export function findState(name: string | null | undefined): IndianState | undefined {
  const k = key(name);
  return k ? INDIA_STATES.find((s) => s.name.toLowerCase() === k) : undefined;
}

export function findDistrict(state: IndianState, name: string | null | undefined): string | undefined {
  const k = key(name);
  return k ? state.districts.find((d) => d.toLowerCase() === k) : undefined;
}

/** The list's spelling of a saved state, or the saved text as-is when it is not in the list. */
export function canonicalStateName(name: string | null | undefined): string {
  return findState(name)?.name ?? (name ?? "").trim();
}

export function isSameState(a: string | null | undefined, b: string | null | undefined): boolean {
  const k = key(canonicalStateName(a));
  return k !== "" && k === key(canonicalStateName(b));
}
